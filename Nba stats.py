...
def PPG(navn):
    for i in navn:
        ...
        PPG = points / games
        print(i, " hadde en PPG average på", round(PPG, 1).values, " i sesongen 2014-15")
...
def FT(navn):
    for i in navn:
        FTmade = stats.loc[lambda stats: stats['Name'] == i, "FTM"]
        FTattempts = stats.loc[lambda stats: stats['Name'] == i, "FTA"]
        FT = FTmade / FTattempts * 100
        print(i, "hadde en straffeprosent på", round(FT, 1).values,"%")

def eFGprosent(navn):
    for i in navn:
        FG = stats.loc[lambda stats: stats['Name'] == i, "FGM"]
        TreP = stats.loc[lambda stats: stats['Name'] == i, "3PM"]
        FGA = stats.loc[lambda stats: stats['Name'] == i, "FGA"]
        eFGprosent = ((FG + (1/2 * TreP)) / FGA) * 100
        print(i, "hadde en effective field goal percentage på", round(eFGprosent, 1).values,"%")
